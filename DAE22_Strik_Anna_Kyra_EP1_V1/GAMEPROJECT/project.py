...
# PROGFA1 EXAM HOWTO
# DEEL 1
# Wilde de timer verdubbelen en de speed verdubbelen wanneer die in cheatmode zit, maar was niet helemaal gelukt toch. had niet genoeg tijd


# DEEL 2
# Heb de navigatie van de startscherm naar het difficulty scherm veranderd naar enter inplaats van alle keys.
# Als de user in het start scherm 'dae' schrijft dan gaat ie meteen naar de highscore screen met als word_count = 999999 als score
# Heb een was_dae_typed function gemaakt die een boolean als return waarde geeft. Deze functie kijkt naar de text variabelen en zoekt naar 'dae'.
# Dit is gedaan doormiddel van de .find() string function
# Deze functie is te vinden, net boven de key_up function

# De was_dae_typed() functie wordt gebruikt in de key up event functie waar die alleen word gebruikt in het Startscherm.

import random
import string #PROGFA1 EXAM
...
class CheatMode(Enum):
    ENABLED = 0,
    DISABLED = 1
current_cheatmode = CheatMode.DISABLED
# -----------------------------------
...
# TIMER
beginning_time = 60 #PROGFA1 EXAM
speed = 2 #PROGFA1 EXAM
timer = 0
seconds = 0
timer_seconds = beginning_time
...
def draw_timer():
    ...
    engine.draw_text(f"time = {int(beginning_time - seconds)}", 40, engine.height - 53) #PROGFA1 EXAM (alleen de beginning_time var)
...
def start_screen():
    ...
    # Shadow
    engine.color = primary_text_clr
    engine.draw_text('Press enter to play_', engine.width - 228, engine.height - 67, True)
    engine.color = highlight_clr
    engine.draw_text('Press enter to play_', engine.width - 225, engine.height - 70, True)
...
def add_random_animal():
    ...
    if timer_animal > speed and len(random_animal_list) < 8:
...
def evaluate():
    ...
    if current_state == GameState.GAMEPLAY:
        ...
        if seconds == beginning_time / 3:
            sunset_color = 0, 1, 1
        elif seconds == beginning_time / 3 * 2:
            sunset_color = 0, 0, 0.502
        elif beginning_time - seconds <= 0:
...
# PROGFA1 EXAM
#-----------------------------------------------------------------------
def was_dae_typed(key: str) -> bool:
    """
    Als DAE wordt getyped in wat voor vorm ook, wordt de return true
    :return: wanneer er dae wordt getyped
    """
    global text
...
def key_up_event(key: str):
    ...
    global current_cheatmode, beginning_time, word_count, faults, text

    if current_state == GameState.START and key == "ENTER":
        current_state = GameState.DIFFICULTY

    # PROGFA1 EXAM
    # -----------------------------------------------------------------------
    if current_state == GameState.START and key:
        if was_dae_typed(key):
            word_count = 999999
            faults = 0
            current_state = GameState.HIGHSCORE
            text = ""
    ...
    if current_state == GameState.GAMEPLAY:
        ...
        if key == "LSHIFT" and current_cheatmode == CheatMode.DISABLED:
            current_cheatmode = CheatMode.ENABLED
            print(f"cheatmode on {current_cheatmode}")
            beginning_time *= 2
            speed = 4
            print(beginning_time)

        elif key == "LSHIFT" and current_cheatmode == CheatMode.ENABLED:
            current_cheatmode = CheatMode.DISABLED
            print(f"cheatmode off {current_cheatmode}")
            beginning_time /= 2
            speed = 2
            print(beginning_time)
