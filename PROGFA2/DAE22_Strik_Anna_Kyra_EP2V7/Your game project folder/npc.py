...
class NPC:
    ...
    def __init__(self, name : str, map : Map, engine: ProgfaEngine):
        ...
        self.map = map
        self.name = name
        self.progression = NPC.DialogueProgression.NO
        self._load_dialogue()
    ...
    def draw_dialogue(self):
        if self.progression in (
                NPC.DialogueProgression.NO,
                NPC.DialogueProgression.FINISHED
        ):
            return
        ...
        if self.progression == NPC.DialogueProgression.BEGIN:
            lines = self.dialogue_data["BEGIN"]

        elif self.progression == NPC.DialogueProgression.CONVERSATION:
    ...
    def start_dialogue(self):
        if self.progression == NPC.DialogueProgression.NO:
            self.progression = NPC.DialogueProgression.BEGIN


    def progress_dialogue(self, key):
        if key == "ENTER":
            if self.progression == NPC.DialogueProgression.BEGIN:
                self.progression = NPC.DialogueProgression.CONVERSATION
            elif self.progression == NPC.DialogueProgression.CONVERSATION:
                self.progression = NPC.DialogueProgression.FINISHED
            
    def can_move(self) -> bool:
        if not (self.progression == NPC.DialogueProgression.NO or self.progression == NPC.DialogueProgression.FINISHED):
