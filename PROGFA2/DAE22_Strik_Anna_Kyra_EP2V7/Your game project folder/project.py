...
#PROGFA2 EXAM HOWTO
    # Wat ik allemaal heb veranderd voor dit examen heb ik #EXAM PROGFA achter gezet
    #Ik heb geen trade chain kunnen maken voor men project, dus heb de text file een beetje anders gedaan.
    #Waarnee die een log line maakt zijn bij het volgende: Wanneer Player een invite aan iemand heeft gegeven (einde van de dialogue)
    # en wanneer de speler van map veranderd

    # De gehele text van de project log wordt opgeslagen in de global variabele 'logfile_lines'
    # Wanneer de start screen weer komt wordt 'logfile_lines' weer gewist
    # En tot slot wanneer die naar het endscreen gaat

    # Ik heb men project wel wat beter kunnen maken met meer for loops te gebruiken, nu staat alles door elkaar heen
...
def render():
    ...
    """
    global current_state, current_map, map_dir, logfile_lines
        ...
        if current_map == GameMap.CENTER or current_map == GameMap.LEFT:
            if player.is_out_of_bounds_right():
                if current_map == GameMap.CENTER:
                    ...
                    map_dir = "right"
                    with open(get_logfile(), "w") as file:
                        logfile_lines += f"Sprout went to the 'farm'."
                        file.write(logfile_lines) #EXAM PROGFA2
                elif current_map == GameMap.LEFT:
                    current_map = GameMap.CENTER
                    map_dir = "center"
                    with open(get_logfile(), "w") as file:
                        logfile_lines += f"Sprout went back to the 'square'."
                        file.write(logfile_lines) #EXAM PROGFA2
        if current_map == GameMap.CENTER or current_map == GameMap.RIGHT:
            if player.is_out_of_bounds_left():
                if current_map == GameMap.CENTER:
                    ...
                    map_dir = "left"
                    logfile_lines += f"Sprout went back to the chicken 'farm area'."
                    with open(get_logfile(), "w") as file:
                        file.write(logfile_lines) #EXAM PROGFA2
                elif current_map == GameMap.RIGHT:
                    current_map = GameMap.CENTER
        ...
        if current_map == GameMap.CENTER or current_map == GameMap.BOTTOM:
            if player.is_out_of_bounds_up():
                if current_map == GameMap.CENTER:
                    ...
                    map_dir = "top"
                    logfile_lines += f"Sprout went to the 'forest'."
                    with open(get_logfile(), "w") as file:
                        file.write(logfile_lines) #EXAM PROGFA2
                elif current_map == GameMap.BOTTOM:
                    current_map = GameMap.CENTER
        ...
        if current_map == GameMap.CENTER or current_map == GameMap.UP:
            if player.is_out_of_bounds_down():
                if current_map == GameMap.CENTER:
                    ...
                    map_dir = "bottom"
                    logfile_lines += f"Sprout went back to the 'picknick' place."
                    with open(get_logfile(), "w") as file:
                        file.write(logfile_lines) #EXAM PROGFA2
                elif current_map == GameMap.UP:
                    current_map = GameMap.CENTER
...
def key_up_event(key: str):
    ...
    """
    global current_state, number_invites, logfile_lines
    if current_state == GameState.START and key:
        ...
        for npc in [
            ...
            if not npc.dialogue_done:
                if npc.progression == NPC.DialogueProgression.FINISHED:
                    with open(get_logfile(), "w") as file:
                        logfile_lines += f"Sprout gave an invite to {npc.name} he still needs to give {number_invites} invites!\n"
                        file.write(logfile_lines)
                    npc.dialogue_done = True # EXAM PROGFA

        if number_invites <= 0:
            current_state = GameState.END_SCREEN
            with open(get_logfile(), "w") as file:
                logfile_lines += f"Sprout got to give all his invites!!"
                file.write(logfile_lines)
    elif current_state == GameState.END_SCREEN and key:
        current_state = GameState.START
        number_invites = 7
        logfile_lines = "" #EXAM PROGFA
